...
# Indexes
#
#  index_report_line_times_on_code_id  (code_id)
#  index_report_line_times_on_line_id  (line_id)
#
# Foreign Keys
#
...
#
class ReportLineTime < ApplicationRecord
  belongs_to :line
  belongs_to :code

  # 月(YYYY-MM-DD),ラインIDごとに集計
  scope :monthly_line_report, -> {
    group(:line_id, "TO_CHAR(DATE_TRUNC('month', date), 'YYYY-MM-DD')").sum(:time)
  }
